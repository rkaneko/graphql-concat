...
function findAllFragmentSpreadNodes(documentNode: DocumentNode): FragmentSpreadNode[] {
    return documentNode.definitions.reduce<FragmentSpreadNode[]>((fsns, dn) => {
        if (dn.kind === "OperationDefinition") {
            ...
            return fsns.concat(
                findFragmentSpreadNodesFromSelectionSet(operationDefinitionNode.selectionSet)
            );
        } else if (dn.kind === "FragmentDefinition") {
            const fragmentDefinitionNode = dn as FragmentDefinitionNode;
            return fsns.concat(
                findFragmentSpreadNodesFromSelectionSet(fragmentDefinitionNode.selectionSet)
            );
        }
        return fsns;
    }, []);
